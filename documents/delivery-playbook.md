...
Claude가 Tracker Boot 백로그에서 다음 스토리를 골라 각 Dev 트랙 파일의 `## 현재 스토리`에 **번호·제목 + 트랙 지시만** 쓴다. 상세·AC·Gherkin·TDD를 복붙하지 않는다 — **정본은 트래커이고, Dev는 스토리 시작 시 `tb_get_story`로 상세·AC를 읽는다.**

`commands/ready-<dev>.command` 를 더블클릭한다. Claude Code가 실행되면서 자동으로 브랜치를 최신 main 기준으로 리셋하고 `dev-<dev> 시작` 트리거를 전송한다.
...
PM이 Dev 세션에 **"PR해"**라고 하면, Dev가 `bash commands/pr.sh <track>`를 실행해 **봇 계정 명의로** PR을 연다. PR 결과(번호·URL)를 PM 세션에 붙여넣는다.
...
- **지적** → Claude가 지적 본문을 `.review-body.md`에 작성하고, PM이 **`decline-<dev>.command`를 더블클릭**한다.
- **통과** → PM이 **`approve-<dev>.command`를 더블클릭**해 승인한다.

---

### 5단계 — Merge

PM이 `merge-<dev>.command`를 더블클릭하고 결과를 PM 세션에 붙여넣는다.
